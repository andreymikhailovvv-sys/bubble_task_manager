...
- Для бота должен быть настроен Main Mini App через @BotFather.
- Main Mini App Планировыча использует `${PUBLIC_APP_URL}/miniapp`.
- На поддерживаемых клиентах Telegram сам отображает в системном меню пункт «Создать ярлык».
- Приложение не вызывает `addToHomeScreen` и не показывает собственную кнопку добавления ярлыка.
