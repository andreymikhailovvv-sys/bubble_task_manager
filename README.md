...
- `TELEGRAM_MINI_APP_URL` задаёт адрес Mini App для существующих `web_app`-ссылок бота.
- `TELEGRAM_MENU_BUTTON_URL` опционально задаёт отдельный адрес глобальной кнопки «Список задач» (по умолчанию используется `TELEGRAM_MINI_APP_URL`).
- `TELEGRAM_CONFIGURE_MENU_BUTTON=true` явно разрешает процессу настраивать глобальную кнопку при старте. По умолчанию это выключено; при общем bot token включайте настройку только на одном deployment (например, на Timeweb), а на Render оставляйте `false` или не задавайте.
