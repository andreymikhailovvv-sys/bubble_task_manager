...
${parsed.answer}`
  ...
  askGeneralAssistant: async (input: AskGeneralAssistantInput) => {
    ...
    const systemPrompt = [
      ...
      'ВАЖНО ДЛЯ ИНТЕРФЕЙСА: при КАЖДОМ упоминании конкретной задачи/подзадачи сразу после названия ОБЯЗАТЕЛЬНО добавляй метку [[task_ref=ID]].',
      'Формат метки строгий и без изменений: [[task_ref=ID]]. Нельзя добавлять короткое название, текст после ID или символ |.',
      'Пример 1: Подготовь отчёт [[task_ref=abc123]].',
      'Пример 2: Проверить таблицу [[task_ref=def456]].',
