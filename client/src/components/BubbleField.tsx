...
function getBubbleShade(hex: string, distanceRatio: number) {
  ...
  const fade = 0.08 + distanceRatio * 0.45;
...
export function BubbleField({ tasks, spheres, mode, selectedId, poppingTaskId, onSelect, onRenameSphere, onAddTaskToSphere, className }: Props) {
            ...
            {bubbles.map((bubble) => {
              ...
              return (
                <motion.g
                  ...
                  animate={isPopping ? { opacity: 0, scale: 1.28 } : { opacity: 1, scale: 1, x: bubble.x, y: bubble.y }}
                  exit={{ opacity: 0, scale: 0.5 }}
                  transition={{ type: isPopping ? 'tween' : 'spring', duration: isPopping ? 0.33 : undefined, damping: 24, stiffness: 180 }}
                  whileHover={isPopping ? undefined : { scale: 1.06 }}
