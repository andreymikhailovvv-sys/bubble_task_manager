...
const NOTE_FORMAT_BUTTONS: Array<{
  format: NoteFormat;
  label: string;
  title: string;
  icon: typeof Bold;
}> = [
  {
    format: "h1",
    label: "H1",
    title: "Заголовок первого порядка",
    icon: Heading1,
  },
  {
    format: "h2",
    label: "H2",
    title: "Заголовок второго порядка",
    icon: Heading2,
  },
...
function formatHeading(selectedText: string, level: 1 | 2) {
  ...
  return {
    ...
    selectionEnd: formattedText.length,
  };
}
...
function NotesEditor({ value, onChange, onClose }: NotesEditorProps) {
  ...
  return (
    <div
      className="notes-editor-backdrop fixed inset-0 z-[220] flex items-center justify-center p-4"
      onClick={onClose}
    >
      <section
        className="notes-editor-panel flex h-[min(82vh,680px)] w-full max-w-4xl flex-col rounded-3xl border shadow-2xl"
        ...
        <div className="notes-editor-header flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3">
          <div>
            ...
            <p className="text-xs text-muted">
              Выделите текст и примените простое форматирование.
            </p>
          </div>
          <button
            type="button"
            className="notes-editor-close rounded-full px-3 py-1.5 text-sm font-semibold"
            onClick={onClose}
          >
            Готово
          </button>
...
export function TaskEditor({
  ...
  const [recurrenceSummary, setRecurrenceSummary] = useState<string | null>(
    null,
  );
  const [recurrenceNextDueLabel, setRecurrenceNextDueLabel] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    const nextForm: Partial<Task> = task ?? {
      ...
      aiNotificationsEnabled: isEventEditor
        ? false
        : defaultAiNotificationsEnabled,
      taskType: isEventEditor ? "EVENT" : "TASK",
      location: null,
    };
    setForm(nextForm);
    ...
    setRecurrenceNextDueLabel(
      nextForm.dueDate
        ? new Date(nextForm.dueDate).toLocaleString("ru-RU")
        : null,
    );
    ...
    autosaveSignatureRef.current = task
      ? JSON.stringify({
          ...
          aiNotificationsEnabled:
            task.aiNotificationsEnabled ?? defaultAiNotificationsEnabled,
        })
      : null;
  }, [task, initialSphereId, defaultAiNotificationsEnabled, isEventEditor]);
  ...
  useEffect(() => {
    ...
    const nextSignature = JSON.stringify({
      ...
      aiNotificationsEnabled:
        normalized.aiNotificationsEnabled ?? defaultAiNotificationsEnabled,
    });
  ...
  const canShowAiCreateMode = !isEventEditor && !isEditing && !isSubtask;
  ...
  const handleAiFileSelect = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const normalized = selectedFiles.filter(
      (file) =>
        SUPPORTED_AI_FILE_TYPES.has(file.type) ||
        /\.(pdf|docx|xlsx?|png|jpe?g|webp|gif)$/i.test(file.name),
    );
    ...
    const oversized = normalized.find(
      (file) => file.size > MAX_AI_ATTACHMENT_SIZE,
    );
    if (oversized) {
  ...
  const submitAiGenerate = async () => {
    ...
    try {
      ...
      const attachments = await Promise.all(
        aiPendingFiles.map((file) => fileToAttachmentPayload(file)),
      );
      const autoAssignSphere = aiSphereSelection === "auto";
      const sphereId =
        aiSphereSelection === "none" || aiSphereSelection === "auto"
          ? null
          : aiSphereSelection;
      ...
      setAiError(
        error instanceof Error
          ? error.message
          : "Не удалось сформировать задачу через ИИ",
      );
    } finally {
      setIsGeneratingByAi(false);
    }
  ...
  const visibleSubtasks = subtasks
    ...
    .slice(0, 5);
  const deadlineLabel = form.dueDate
    ? `До дедлайна: ${formatDeadlineLeft(form.dueDate)}`
    : "Дедлайн не задан";
  ...
  const applyRecurrence = async () => {
    ...
    try {
      ...
      setRecurrenceNextDueLabel(
        parsed.nextDueDate
          ? new Date(parsed.nextDueDate).toLocaleString("ru-RU")
          : null,
      );
      setForm((p) => ({
        ...p,
        ...
        <main className="focus-main-card task-edit-card flex h-full min-h-0 flex-col overflow-hidden rounded-[2rem] bg-white p-3">
          ...
          {canShowAiCreateMode ? (
            <div className="mt-3 grid grid-cols-2 gap-2">
              <button
                className={`rounded-full px-3 py-2 text-sm font-semibold ${createMode === "manual" ? "primary-button" : "secondary-button"}`}
                onClick={() => setCreateMode("manual")}
              >
                Вручную
              </button>
              <button
                className={`ai-create-mode-button rounded-full border px-3 py-2 text-sm font-semibold transition ${createMode === "ai" ? "ai-create-mode-button-active" : ""}`}
          ...
          {createMode === "ai" && canShowAiCreateMode ? (
            <div className="mt-4 flex min-h-0 flex-1 flex-col gap-3">
              <p className="text-xs text-muted">
                Опишите задачу в свободной форме — ИИ сам заполнит название,
                описание, сроки, степень важности и даст подсказки{" "}
                <span className="inline-flex items-center gap-1 text-rose-300">
                  (стоимость 2 <Coins size={11} />)
                </span>
              </p>
              <textarea
                className="form-field min-h-0 flex-1 resize-none rounded-2xl border p-3 text-sm"
                placeholder="Например: нужно подготовить презентацию…"
                value={aiPrompt}
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {aiPendingFiles.map((file) => (
                  <button
                    ...
                    type="button"
                    className="task-attachment-pill inline-flex max-w-[220px] items-center gap-1 rounded-full border px-2 py-1 text-[11px]"
                    onClick={() =>
                      setAiPendingFiles((prev) =>
                        prev.filter(
                          (item) =>
              ...
              <CustomSelect
                ...
                options={[
                  { value: "auto", label: "Автоматически", color: "#0ea5e9" },
                  { value: "none", label: "Без сектора", color: "#7c3aed" },
                  ...spheres.map((sphere) => ({
                    value: sphere.id,
                    label: sphere.name,
                    color: sphere.color,
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <button
                  className="primary-button rounded-full px-4 py-2 text-sm disabled:opacity-60"
            ...
            <div className="invisible-scrollbar flex min-h-0 flex-1 flex-col overflow-y-auto pr-1">
              ...
              <input
                ref={aiAttachmentInputRef}
                type="file"
                accept=".pdf,.docx,.xls,.xlsx,.png,.jpg,.jpeg,.webp,.gif"
                multiple
                className="hidden"
                onChange={handleAiFileSelect}
              />
              {isSubtask && parentTaskTitle && onOpenParentTask ? (
                <button
                  type="button"
                  className="main-task-link-button mt-2 inline-flex max-w-full min-w-0 rounded-full border px-3 py-1 text-xs font-semibold transition"
              ...
              {!isEventEditor && !isSubtask ? (
                <div className="mt-3 pt-1">
                  <p className="mb-2 text-xs font-semibold uppercase tracking-[0.14em] text-violet-500">
                    Важность: {selectedImportance}
                  </p>
                  <div className="importance-choice-group grid grid-cols-5 gap-2">
                    {[1, 2, 3, 4, 5].map((level) => (
                      <button
                        key={level}
                        className={`importance-choice-button task-edit-importance rounded-xl border px-2 py-2 text-sm font-semibold transition ${IMPORTANCE_STYLES[level]} ${selectedImportance === level ? "importance-choice-button-active ring-2" : "opacity-80 hover:opacity-100"}`}
                        onClick={() =>
                          setForm((p) => ({ ...p, importance: level }))
                        }
                      >
                        {level}
                      </button>
                ...
                <div className="mt-auto pt-4">
                  ...
                  <div className="importance-choice-group grid grid-cols-3 gap-2">
                    {SUBTASK_IMPORTANCE_OPTIONS.map(({ level, label }) => (
                      <button
                        key={level}
                        type="button"
                        title={label}
              ...
              {isRecurring ? (
                <div className="surface-card mt-2 rounded-2xl border p-2 text-xs">
                  ...
                  <textarea
                    ...
                    placeholder="Например: каждый вторник и четверг в 17:00"
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <button
                      type="button"
                      className="recurrence-send-button rounded px-2 py-1 text-xs font-semibold disabled:opacity-70"
                      onClick={() => void applyRecurrence()}
                  ...
                  <p className="mt-1 text-[11px] text-muted">
                    {recurrenceNextDueLabel
                      ? `Ближайший срок: ${recurrenceNextDueLabel}`
                      : ""}
                  </p>
                </div>
              ) : null}
              {!isEventEditor && !isEditing && !isSubtask ? (
                <div className="focused-task-subtasks mt-3 flex min-h-0 flex-col space-y-2 border-t border-violet-100 pt-3">
                  <div className="flex items-center justify-between gap-2">
                  ...
                  {draftSubtasks.length > 0 ? (
                    ...
                    <p className="text-xs text-subtle">Пока нет подзадач</p>
                  )}
                </div>
              ) : null}
              {isEditing ? (
                <div className="mt-auto flex gap-2 pt-4">
                  <button
                    type="button"
                    className="success-button inline-flex flex-1 items-center justify-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold"
                    onClick={() => onComplete?.()}
                  >
                  ...
                  <button
                    className="danger-button inline-flex flex-1 items-center justify-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold"
                    onClick={() => onDelete?.()}
                  >
                    <Trash2 size={15} />
                    Удалить
