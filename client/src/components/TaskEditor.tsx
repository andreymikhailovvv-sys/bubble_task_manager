        ...
        <main className="focus-main-card task-edit-card flex h-full min-h-0 flex-col overflow-hidden rounded-[2rem] bg-white p-3">
          ...
          {createMode === 'ai' && canShowAiCreateMode ? (
            <div className="mt-4 flex min-h-0 flex-1 flex-col gap-3">
              ...
              <CustomSelect value={aiSphereSelection} onChange={setAiSphereSelection} options={[{ value: 'auto', label: 'Автоматически', color: '#7c3aed' }, { value: 'none', label: 'Без сектора', color: '#7c3aed' }, ...spheres.map((sphere) => ({ value: sphere.id, label: sphere.name, color: sphere.color }))]} ariaLabel="Выбор сектора для ИИ" buttonClassName="focused-task-pill-select" menuClassName="task-edit-sector-menu" detachedPopup />
