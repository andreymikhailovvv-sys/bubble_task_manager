        ...
        <main className="focus-main-card task-edit-card flex h-full min-h-0 flex-col overflow-hidden rounded-[2rem] bg-white p-3">
          ...
          {createMode === 'ai' && canShowAiCreateMode ? (
            <div className="mt-4 flex min-h-0 flex-1 flex-col gap-3">
              ...
              <CustomSelect value={aiSphereSelection} onChange={setAiSphereSelection} options={[{ value: 'auto', label: 'Автоматически' }, { value: 'none', label: 'Без сектора' }, ...spheres.map((sphere) => ({ value: sphere.id, label: sphere.name }))]} ariaLabel="Выбор сектора для ИИ" detachedPopup />
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              <div className="mt-2 flex items-start justify-between gap-3">
                <input className="task-edit-title-input min-w-0 flex-1 border-0 bg-transparent p-0 text-3xl font-bold text-slate-950 shadow-none outline-none" placeholder="Введите название" value={form.title ?? ''} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} />
              </div>
              ...
              <div className="mt-3 flex h-32 min-h-32 items-start rounded-2xl bg-slate-50/70 p-3">
                {isEditing ? (
                  ...
                  <textarea className="min-h-0 w-full flex-1 resize-none border-0 bg-transparent text-sm leading-6 text-slate-700 outline-none placeholder:text-slate-400" placeholder="Введите описание" value={descriptionValue} onChange={(e) => updateDescription(e.target.value)} />
                )}
              </div>
              ...
              {!isEditing && !isSubtask ? <div className="mt-4 rounded-2xl bg-slate-50/70 p-3"><div className="mb-2 flex items-center justify-between"><h3 className="text-sm font-semibold text-primary">Подзадачи</h3><span className="text-xs text-subtle">{draftSubtasks.length}</span></div><div className="flex gap-2"><input className="form-field min-w-0 flex-1 rounded border px-2 py-1.5 text-xs" placeholder="Название доп задачи" value={draftSubtaskTitle} onChange={(e) => setDraftSubtaskTitle(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addDraftSubtask(); } }} /><button type="button" className="focused-task-add-subtask-button" onClick={addDraftSubtask} title="Добавить подзадачу" aria-label="Добавить подзадачу"><Plus size={15} /></button></div>{draftSubtasks.length > 0 ? <ul className="mt-2 max-h-20 space-y-1 overflow-y-auto text-xs text-slate-700">{draftSubtasks.map((subtask, index) => <li key={`${subtask.title}-${index}`} className="flex items-center gap-2 rounded-xl bg-white px-2 py-1.5"><span className="min-w-0 flex-1 truncate">{subtask.title}</span><button type="button" className="text-slate-400 hover:text-rose-500" onClick={() => setDraftSubtasks((prev) => prev.filter((_, itemIndex) => itemIndex !== index))}><X size={12} /></button></li>)}</ul> : null}</div> : null}
