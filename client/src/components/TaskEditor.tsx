      ...
      <aside className="w-full max-w-xl space-y-3 rounded-2xl border border-slate-700/50 bg-slate-900 p-4" onClick={(e) => e.stopPropagation()}>
        ...
        {canShowAiCreateMode ? (
          <div className="grid grid-cols-2 gap-2">
            ...
            <button className={`rounded px-3 py-2 text-sm font-semibold ${createMode === 'ai' ? 'bg-rose-500 text-white' : 'bg-slate-800 text-slate-200'}`} onClick={() => setCreateMode('ai')}>Через ИИ</button>
          </div>
        ) : null}
