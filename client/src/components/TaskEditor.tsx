        ...
        <main className="focus-main-card task-edit-card flex h-full min-h-0 flex-col overflow-hidden rounded-[2rem] bg-white p-3">
          ...
          {createMode === 'ai' && canShowAiCreateMode ? (
            <div className="mt-4 flex min-h-0 flex-1 flex-col gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2"><button type="button" className="secondary-button inline-flex h-9 w-9 items-center justify-center rounded-xl border border-dashed text-primary hover:brightness-110" onClick={() => aiAttachmentInputRef.current?.click()} title="Добавить файл"><Plus size={15} /></button>{aiPendingFiles.map((file) => <button key={`${file.name}:${file.size}`} type="button" className="task-attachment-pill inline-flex max-w-[220px] items-center gap-1 rounded-full border px-2 py-1 text-[11px]" onClick={() => setAiPendingFiles((prev) => prev.filter((item) => `${item.name}:${item.size}` !== `${file.name}:${file.size}`))}><Paperclip size={11} /><span className="truncate">{file.name}</span><X size={11} /></button>)}</div>
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              ...
              <div className="focused-task-description-surface mt-3 flex h-32 min-h-32 items-start rounded-2xl bg-slate-50/70 p-3" onClick={() => subtaskDescriptionInputRef.current?.focus()}>
                {isEditing && !isSubtask ? (
                  ...
                  <textarea ref={subtaskDescriptionInputRef} className="subtask-description-inline invisible-scrollbar h-full min-h-0 w-full flex-1 resize-none overflow-y-auto border-0 bg-transparent text-sm leading-6 text-muted outline-none placeholder:text-slate-400" placeholder="Введите описание" value={noteHtmlToPlainText(descriptionValue, { trimEnd: false })} onChange={(e) => updateDescription(e.target.value)} />
                )}
              </div>
              <div className="mt-2 flex flex-wrap items-center gap-2"><button type="button" className="focused-task-icon-button inline-flex h-8 w-8 items-center justify-center rounded-full border transition" onClick={() => setIsNotesEditorOpen(true)} title="Редактировать описание"><Edit3 size={15} /></button><button type="button" className="focused-task-icon-button inline-flex h-8 w-8 items-center justify-center rounded-full border transition" onClick={() => aiAttachmentInputRef.current?.click()} title="Добавить файл"><Plus size={15} /></button>{aiPendingFiles.map((file) => <button key={`${file.name}:${file.size}`} type="button" className="task-attachment-pill inline-flex max-w-[220px] items-center gap-1 rounded-full border px-2 py-1 text-[11px]" onClick={() => setAiPendingFiles((prev) => prev.filter((item) => `${item.name}:${item.size}` !== `${file.name}:${file.size}`))}><Paperclip size={11} /><span className="truncate">{file.name}</span><X size={11} /></button>)}</div>
