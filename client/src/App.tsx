        ...
        <div className="modal-backdrop fixed inset-0 z-[140] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setIsAiChatOpen(false)}>
          <div className="focus-mode-shell grid h-[min(820px,calc(100vh-32px))] w-full max-w-6xl grid-cols-[280px_minmax(0,1fr)] overflow-hidden rounded-3xl border shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <section className="flex min-h-0 flex-col p-5">
              ...
              <div className="ai-chat-thread-wrap relative mt-3 min-h-0 flex-1 pt-5">
                <CustomSelect value={selectedAiChatModel} options={AI_CHAT_MODEL_SELECT_OPTIONS} onChange={(value) => setSelectedAiChatModel(value as AiChatModel)} className="ai-chat-model-cap absolute left-5 top-0 z-10 w-44" buttonClassName="ai-chat-model-cap-button rounded-t-2xl rounded-b-none border px-3 py-1.5 text-xs font-semibold text-primary shadow-sm hover:brightness-105" menuClassName="ai-chat-model-cap-menu surface-popover text-primary" ariaLabel="Выбрать модель чата" />
