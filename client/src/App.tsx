...
function AiModelChip({ value, onChange, ariaLabel, className = '' }: { value: AiChatModel; onChange: (value: AiChatModel) => void; ariaLabel: string; className?: string }) {
  return (
    <div className={`ai-chat-model-cap w-40 ${className}`}>
  ...
  const renderTimelineTaskChip = (task: Task, options?: { showTime?: boolean; isSubtask?: boolean; disableHoverCard?: boolean; parentTaskTitle?: string; disableEffects?: boolean; disableOpenOnClick?: boolean; forceDraggable?: boolean; onDragStart?: () => void; compact?: boolean }) => {
    ...
    return (
      <motion.button
        ...
        className={`timeline-task-chip ${isEventChip ? 'timeline-event-chip' : ''} relative flex w-full items-center justify-between border text-left transition-all duration-200 hover:brightness-110 ${options?.compact ? 'gap-1 rounded px-1.5 py-0.5 text-[10px] leading-3' : 'gap-2 rounded-md px-2 py-1 text-xs'} ${
      ...
      >
        <span className="flex min-w-0 items-center gap-1">
          ...
          {timelinePostponeLoadingTaskId === task.id ? <Loader2 size={options?.compact ? 11 : 12} className="timeline-task-chip-accent shrink-0 animate-spin" /> : null}
          {isEventChip ? <Ticket size={options?.compact ? 11 : 12} className="shrink-0 text-amber-600" /> : null}
          {isSubtaskChip ? <span className={`${options?.compact ? 'h-3.5 w-px' : 'h-4 w-1'} shrink-0 rounded-sm`} style={{ backgroundColor: parentSphereColor }} /> : null}
          ...
          {!isEventChip && hasUnreadAiMessage(task.id) ? <span title="Непрочитанное ИИ-уведомление"><Sparkles size={options?.compact ? 11 : 12} className="timeline-task-ai-icon shrink-0" /></span> : null}
          ...
          {task.isRecurring ? <span title="Повторяющееся событие/задача"><Repeat size={options?.compact ? 11 : 12} className="timeline-task-chip-accent shrink-0" /></span> : null}
        ...
        <div className="fixed inset-0 z-[151] flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm" onClick={() => setIsFocusAiExpanded(false)}>
          <div className="dialog-surface relative flex h-[90vh] w-full max-w-4xl flex-col rounded-3xl border p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="ai-chat-composer mt-3 flex items-center gap-2 rounded-3xl border p-2 shadow-lg backdrop-blur">
              ...
              <button className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-violet-600 text-white shadow-lg disabled:opacity-50" title="Отправить" disabled={!isFocusTimerRunning || focusAiLoading || (!focusAiDraft.trim() && focusAiPendingFiles.length === 0)} onClick={() => void sendFocusAiQuestion()}>{focusAiLoading ? <Loader2 className="animate-spin" size={16} /> : <SendHorizontal size={16} />}</button>
            </div>
          </div>
        </div>
      ) : null}
                ...
                <section className="timeline-panel rounded-2xl border p-3">
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {timelineViewData.monthCells.map((cell) => (
                      <div
                        key={cell.key}
                        className={`timeline-month-cell h-24 overflow-hidden rounded-xl border p-1 ${
                          cell.date
                      ...
                      >
                        {cell.date ? (
                          <>
                            <p className="mb-1 flex items-center justify-between text-[11px] font-semibold leading-3 text-muted">
